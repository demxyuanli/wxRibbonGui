...
void FlatUIBarStateManager::SaveActivePageBeforeUnpin()
{
    ...
    LOG_INF("Saved active page " + std::to_string(m_lastPinnedActivePage) + " before unpinning", "StateManager");
}

void FlatUIBarStateManager::RestoreActivePageAfterPin()
{
    m_activePage = m_lastPinnedActivePage;
    LOG_INF("Restored active page " + std::to_string(m_activePage) + " after pinning", "StateManager");
}
...
void FlatUIBarStateManager::ValidateActivePageIndex(size_t totalPages)
{
    ...
    if (m_activePage >= totalPages) {
        ...
        LOG_INF("Active page index was invalid, reset to 0", "StateManager");
    }
